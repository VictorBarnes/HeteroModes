"""Optimize rsfMRI reconstruction AUC with optional anisotropic search.

This script keeps the tfMRI optimizer's parameter handling, caching, and run-folder
bookkeeping, but swaps the objective for rsfMRI functional-connectivity
reconstruction error from ``EigenSolver.reconstruct_timeseries``.

The optimization target is the AUC of the mean subject-wise ``fc_recon_error`` curve.
K@target is reported as a diagnostic only.
"""
...
DEFAULT_BOLD_FILE = Path(PROJ_DIR) / "data" / "empirical" / "human" / "hcp-s1200_nsubj-255_desc-bold_space-fsLR_den-4k_hemi-L_nt-1200.h5"
DEFAULT_ALPHA = None
DEFAULT_BETA = None
DEFAULT_ANISO_CURV1 = None
DEFAULT_ANISO_CURV2 = None
OBJECTIVE_VERSION = "rsfmri_fc_recon_auc_v1"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--error_target", type=float, default=0.5, help="Absolute fc_recon_error target for K@target (target >= 0).")
...
def load_bold_timeseries(n_subjs: int | None, n_timepoints: int | None) -> np.ndarray:
    ...
    with h5py.File(DEFAULT_BOLD_FILE, "r") as f:
        if "bold" not in f:
            raise KeyError(f"Key 'bold' not found in {DEFAULT_BOLD_FILE}")
    ...
    if n_timepoints is not None:
        if n_timepoints <= 0:
            raise ValueError("--n_timepoints must be > 0")
        if n_timepoints > bold_vts.shape[1]:
...
def _subject_fc_recon_errors(
    ...
    return np.stack(subj_curves, axis=1)
...
class ObjectiveEvaluator:
    ...
    def __call__(self, x: np.ndarray) -> float:
        ...
        try:
            ...
            subj_fc_err = _subject_fc_recon_errors(solver, self.bold_data, self.mode_counts)
            curve = np.mean(subj_fc_err, axis=1)
            curve_std = np.std(subj_fc_err, axis=1)
            ...
            payload.update(
                {
                    ...
                    "curve_std": curve_std.astype(np.float32),
...
def evaluate_solver(
    ...
    subj_fc_err = _subject_fc_recon_errors(solver, bold_data, mode_counts)
    curve = np.mean(subj_fc_err, axis=1)
...
def evaluate_solver_stats(
    ...
    subj_fc_err = _subject_fc_recon_errors(solver, bold_data, mode_counts)
    mean_curve = np.mean(subj_fc_err, axis=1)
    std_curve = np.std(subj_fc_err, axis=1)
...
def plot_auc_landscape(
    *,
    ...
    if len(free_param_names) == 1:
        ...
        ax_k.set_xlabel(p0)
        ax_k.set_ylabel(f"K@fc_err<={float(error_target):.3f}")
    ...
    save_path = run_dir / "landscape.png"
...
def plot_best_reconstruction_curves(
    *,
    ...
    k_label = f"K@fc_err<={float(error_target):.3f}"
    ...
    ax.set_ylabel("FC reconstruction error")
    ax.legend(loc="upper right")
    ax.set_title("Best Reconstruction Curves")
    fig.tight_layout()

    save_path = run_dir / "best-recon-curv.png"
    fig.savefig(save_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    return save_path
...
def _compute_isotropic_reconstruction(
    *,
    ...
    curve, auc, k_target = evaluate_solver(solver, bold_data, mode_counts, error_target)
    payload = {
        **meta_base,
        "baseline": "isotropic",
        "cache_key": cache_key,
...
def main() -> None:
    ...
    if float(args.error_target) < 0.0:
        raise ValueError("--error_target must satisfy target >= 0")
    args.aniso_label = _resolved_label(args.aniso_label, args.hetero_label)

    results_dir = Path(PROJ_DIR) / "results" / "human" / "reconstruction" / "rsfmri"
    ...
    mode_counts = np.array([5, 10, 20, 50, 100]) #np.arange(2, args.n_modes + 1, args.mode_step, dtype=int)
    mesh, _ = fetch_surf(density=args.density)
    medmask = nib.load(
        f"{PROJ_DIR}/data/empirical/human/space-fsLR_den-{args.density}_"
    ...
    print("Computing isotropic reconstruction baseline...")
    ...
    if best_cache_path.exists():
        ...
        best_curve, best_auc, best_k_target = evaluate_solver(aniso_solver_for_best, bold_data, mode_counts, float(args.error_target))
        _atomic_savez(
            best_cache_path,
            objective=best_auc,
            auc=best_auc,
