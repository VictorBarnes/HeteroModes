...
medmask = nib.load(
    f"{PROJ_DIR}/data/empirical/human/space-fsLR_den-4k_hemi-L_desc-nomedialwall.func.gii"
).darrays[0].data.astype(bool)

hc_ids = np.loadtxt(PROJ_DIR / "data" / "hcp-ep" / "ids_hc_run-1.txt").astype(int)
scz_ids = np.loadtxt(PROJ_DIR / "data" / "hcp-ep" / "ids_scz_run-1.txt").astype(int)
...
for i, cohort in enumerate(["hc", "scz"]):
    ...
    nib.save(
        gii, PROJ_DIR / "data" / "hcp-ep" / f"cohort-{cohort}_avg-myelinmap.gii"
    )

#%% load computed maps and plot

t1t2_hc = nib.load(PROJ_DIR / "data" / "hcp-ep" / "cohort-hc_avg-myelinmap.gii").darrays[0].data
t1t2_scz = nib.load(PROJ_DIR / "data" / "hcp-ep" / "cohort-scz_avg-myelinmap.gii").darrays[0].data
